...
.PARAMETER Value
    The value to set for the environment variable (passed as a SecureString).
...
.EXAMPLE
    .\Update-WebAppEnvVar.ps1 -TenantID "your-tenant-id" `
                              ...
                              -Value (ConvertTo-SecureString "NewValue123" -AsPlainText -Force) `
                              -Setting "MY_ENV_VAR" `
                              -Slot "preview"
...
Try {
  Write-Debug "Converting SecureString Value to plain text."
  $PlainValue = [System.Net.NetworkCredential]::new("", $Value).Password
  Write-Debug "Conversion successful."
}
Catch {
...
$appSettingsHashtable[$Setting] = $PlainValue

Try {
  Write-Debug "Applying updated application settings."
  Set-AzWebAppSlot -ResourceGroupName $ResourceGroup -Name $WebAppName -AppSettings $appSettingsHashtable -Slot $Slot -ErrorAction 'Stop'
